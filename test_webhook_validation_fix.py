    ...
    for name, url in endpoints:
        try:
            ...
            if response.status_code == 200:
                ...
                if "test" in url:
                    data = response.json()
                    print(f"      Notion: {'✅' if data.get('notion', {}).get('success') else '❌'}")
                    print(f"      Gemini: {'✅' if data.get('gemini', {}).get('success') else '❌'}")
                    print(f"      Telegram: {'✅' if data.get('telegram', {}).get('success') else '❌'}")
            else:
                results[name] = False
