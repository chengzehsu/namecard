...
from src.namecard.core.services.batch_image_collector import get_batch_collector, PendingImage
...
async def test_batch_collector():
    ...
    async def mock_progress_notifier(user_id: str, chat_id: int, image_count: int, action: str):
        """模擬進度通知器"""
    ...
    for attempt in range(60):  # 最多等待6秒
        ...
        if attempt % 10 == 0 and attempt > 0:
            ...
            if current_status:
                print(f"📊 等待中... ({attempt/10:.0f}s) 狀態: 圖片數={current_status['image_count']}, "
    ...
    if batch_result['image_count'] != expected_image_count:
        print(f"❌ 批次圖片數量不正確: 期望 {expected_image_count}, 實際 {batch_result['image_count']}")
        return False
