...
class EnhancedSessionManager:
    ...
    def start_batch_mode(self, user_id: str) -> Dict:
        ...
        with self.lock:
            ...
            if user_id in self.user_sessions:
                ...
                if current_time - existing_session.get('last_activity', current_time) < self.session_timeout:
                    # 恢復現有會話
                    ...
                    return {
                        ...
                        "recovered": True,
                        "stats": {
                            "processed": len(existing_session.get('processed_cards', [])),
    ...
    def get_session_health(self, user_id: str) -> Dict:
        ...
        with self.lock:
            ...
            # 檢查會話是否過期
            if current_time - session.get('last_activity', current_time) > self.session_timeout:
    ...
    def get_system_stats(self) -> Dict:
        ...
        with self.lock:
            ...
            total_processed = sum(len(s.get('processed_cards', [])) for s in self.user_sessions.values())
            total_failed = sum(len(s.get('failed_cards', [])) for s in self.user_sessions.values())
