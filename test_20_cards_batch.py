...
def test_concurrent_capacity():
    ...
    try:
        ...
        with patch('simple_config.Config.TELEGRAM_BOT_TOKEN', 'dummy_token'):
            ...
            if hasattr(handler, '_http_client') and handler._http_client:
                if hasattr(handler._http_client, '_limits'):
                    ...
                    http_config = {
                        ...
                        'max_keepalive_connections': getattr(limits, 'max_keepalive_connections', 'N/A'),
                    }
            ...
            print(f"   HTTP 保持連接數: {http_config.get('max_keepalive_connections', 'N/A')}")
            ...
            checks = [
                ...
                ("保持連接數 >= 40", http_config.get('max_keepalive_connections', 0) >= 40),
        ...
        try:
            ...
            with patch('simple_config.Config.TELEGRAM_BOT_TOKEN', 'dummy_token'):
                ...
                if successful_results:
                    avg_processing = sum(r['processing_time'] for r in successful_results) / len(successful_results)
                ...
                print(f"   時間表現: {'✅ 優秀' if total_time <= 1.0 else '✅ 良好' if total_time <= 3.0 else '⚠️ 需改進'}")
...
def test_rate_limit_behavior():
    ...
    try:
        ...
        with patch('simple_config.Config.TELEGRAM_BOT_TOKEN', 'dummy_token'):
            ...
            # 分析結果
            allowed_requests = sum(1 for can_proceed, _ in request_results if can_proceed)
            blocked_requests = sum(1 for can_proceed, _ in request_results if not can_proceed)
