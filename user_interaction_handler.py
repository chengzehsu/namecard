...
class UserInteractionHandler:
    ...
    def create_multi_card_session(self, user_id: str, analysis_result: Dict[str, Any]) -> str:
        """
        創建多名片處理會話
    ...
    def handle_user_choice(self, user_id: str, user_input: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 解析用戶選擇
            choice_result = self._parse_user_choice(user_input, session["analysis_result"])
    ...
    def _generate_choice_message(self, analysis_result: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if good_cards and poor_cards:
                ...
                message += f"⚠️ 需要改善：第 {', '.join(map(str, poor_cards))} 張名片\n\n"
            ...
            if user_options:
                ...
                message += "\n💬 **請回覆選項編號 (1-{}) 或直接回覆選項內容**".format(len(user_options))
    ...
    def _parse_user_choice(self, user_input: str, analysis_result: Dict[str, Any]) -> Dict[str, Any]:
        """解析用戶的選擇輸入"""
        try:
            user_options = analysis_result.get("user_options", [])
            ...
            try:
                ...
                if 1 <= choice_num <= len(user_options):
                    ...
                    return self._process_selected_option(selected_option, analysis_result)
            except ValueError:
                pass
            ...
            # 檢查常見的同義詞
            if any(keyword in user_input_lower for keyword in ["重拍", "重新", "再拍", "retake"]):
                retake_options = [opt for opt in user_options if "重新拍攝" in opt]
                if retake_options:
                    return self._process_selected_option(retake_options[0], analysis_result)
            
            if any(keyword in user_input_lower for keyword in ["繼續", "處理", "確定", "ok", "continue"]):
                continue_options = [opt for opt in user_options if "繼續處理" in opt or "分別處理" in opt]
                if continue_options:
                    return self._process_selected_option(continue_options[0], analysis_result)
    ...
    def _is_option_match(self, user_input: str, option: str) -> bool:
        ...
        return any(keyword in user_input for keyword in option_keywords if len(keyword) > 1)
    
    def _process_selected_option(self, selected_option: str, analysis_result: Dict[str, Any]) -> Dict[str, Any]:
        """處理用戶選擇的選項"""
        try:
            if "重新拍攝" in selected_option:
                ...
                good_card_data = [all_cards[i-1] for i in good_cards if i <= len(all_cards)]
