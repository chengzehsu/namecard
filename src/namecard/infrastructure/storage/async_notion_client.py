...
class AsyncNotionManager:
    ...
    async def _create_record_with_timeout(
        ...
        return await asyncio.wait_for(
            self._create_record_core(card_data, image_bytes),
            timeout=timeout
        )
    ...
    async def _create_record_core(
        ...
        try:
            ...
            if image_bytes:
                tasks.append(
                    self._add_image_info_async(page_id, image_bytes)
                )
    ...
    async def _build_properties_async(self, card_data: Dict[str, Any]) -> Dict[str, Any]:
        """異步建構 Notion 頁面屬性"""
                ...
                },
                {
                    ...
                    "bulleted_list_item": {
                        "rich_text": [
                            {
                                "type": "text",
                                "text": {"content": f"圖片大小: {image_size_kb:.1f} KB"}
                            }
                        ]
                    }
                }
    ...
    async def _add_address_processing_info_async(self, page_id: str, card_data: Dict[str, Any]):
        """異步添加地址處理資訊"""
        try:
                ...
                },
                {
                    ...
                    "bulleted_list_item": {
                        "rich_text": [
                            {
                                "type": "text",
                                "text": {"content": f"處理信心度: {address_confidence:.1%}"}
                            }
                        ]
                    }
                }
    ...
    async def create_batch_records(
        ...
        async def create_single_record(index: int, card_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 創建所有任務
        tasks = [
            create_single_record(i, card_data)
    ...
    async def get_performance_stats(self) -> Dict[str, Any]:
        ...
        success_rate = (
            ...
            if (self.stats["total_created"] + self.stats["total_errors"]) > 0 else 0.0
        )
