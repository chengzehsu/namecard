...
class BatchProcessingIntegration:
    ...
    async def process_batch_cards(
        self, user_id: str, card_images: List[bytes], batch_session: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        處理批次名片 - 使用異步訊息排程
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                failed_results.append({"index": i + 1, "error": str(result)})
        ...
        return {
            ...
            "success_rate": (
                len(successful_results) / len(card_images) if card_images else 0
            ),
            "results": successful_results,
            "errors": failed_results,
        }

    async def _process_single_card_with_feedback(
        self, user_id: str, image_data: bytes, card_index: int, total_cards: int
    ) -> Dict[str, Any]:
        """
        處理單張名片並提供進度反饋
    ...
    async def _send_batch_completion_summary(
        self,
        user_id: str,
...
"""
        ...
        # 發送完成統計（高優先級，不合併）
        await self.telegram_handler.send_completion_message(
            chat_id=user_id, text=summary_text, parse_mode="Markdown"
        )
    ...
    async def _send_failure_details(self, user_id: str, failed_results: List[Dict]):
        ...
        await self.telegram_handler.send_urgent_message(
            chat_id=user_id, text=failure_text, parse_mode="Markdown"
        )
...
class BatchProgressTracker:
    ...
    async def update_progress(
        ...
        # 每20%進度或最後一個發送更新
        should_update = (
            progress_percent - last_percent >= 20
            or current == total
            or current == 1  # 第一個
        )
        ...
        if should_update:
            ...
            # 使用批次訊息發送進度更新
            await self.telegram_handler.send_batch_message(
                chat_id=user_id, text=progress_text, parse_mode="Markdown"
            )
...
# 🔧 工廠函數
async def create_batch_processing_system(
    telegram_handler: Optional[EnhancedTelegramBotHandler] = None,
