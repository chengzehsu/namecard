...
class TelegramBotHandler:
        ...
        try:
            if not Config.TELEGRAM_BOT_TOKEN:
                ...
                if hasattr(self, "_test_mode") and self._test_mode:
                    self.logger.warning(
                        "⚠️ 測試模式：無 TELEGRAM_BOT_TOKEN，跳過 Bot 初始化"
                    )
                    self.bot = None
            ...
            self._http_client = httpx.AsyncClient(
                ...
                transport=httpx.HTTPTransport(
                    retries=5, verify=True  # 增加傳輸層重試次數  # SSL 驗證
                ),
            )

            # 使用自定義 HTTP 客戶端創建 Bot
            self.bot = ExtBot(token=Config.TELEGRAM_BOT_TOKEN)
    ...
    async def _get_semaphore(self):
        ...
        try:
            ...
            if (
                ...
                or hasattr(self._semaphore, "_loop")
                and self._semaphore._loop != current_loop
            ):
    ...
    def _handle_telegram_error(
        ...
        if isinstance(error, RetryAfter):
            ...
            if (
                "Pool timeout" in error_message
                or "connection pool" in error_message.lower()
            ):
                self._connection_pool_stats["pool_timeouts"] += 1
                self.logger.warning(
                    f"🚨 連接池超時檢測到，總計: {self._connection_pool_stats['pool_timeouts']}"
                )
        ...
        async with semaphore:
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if (
                        not error_result.get("can_retry", False)
                    ...
                    wait_time = self._calculate_retry_delay(
                        attempt, error_result.get("retry_after", 0)
                    )
                    self.logger.info(
                        f"📡 重試 {attempt + 1}/{self.max_retries}，等待 {wait_time:.1f} 秒..."
                    )
    ...
    def get_status_report(self) -> Dict[str, Any]:
        ...
        pool_timeout_ratio = (
            (self._connection_pool_stats["pool_timeouts"] / max(1, total_errors))
            if total_errors > 0
            else 0
        )
        ...
        return {
            ...
            "status": (
                "healthy"
                if total_errors < 10 and pool_timeout_ratio < 0.1
                else (
                    "degraded"
                    if total_errors < 50 and pool_timeout_ratio < 0.3
                    else "unhealthy"
                )
            ),
            "recommendations": self._get_health_recommendations(
                total_errors, pool_timeout_ratio
            ),
        }

    def _get_health_recommendations(
        self, total_errors: int, pool_timeout_ratio: float
    ) -> list:
        """🚀 根據狀態提供健康建議"""
        recommendations = []
            ...
            if hasattr(self, "_http_client") and self._http_client:
                try:
                    ...
                    if hasattr(self._http_client, "aclose"):
                        ...
                        self.logger.debug(
                            f"⚠️ 未知客戶端類型: {type(self._http_client)}"
                        )
    ...
    async def get_connection_pool_metrics(self) -> Dict[str, Any]:
        ...
        try:
                        ...
                        if hasattr(transport, "_pool"):
                            ...
                            pool_info = {
                                ...
                                "available_connections": getattr(
                                    pool, "_available_connections", "unknown"
                                ),
                                "active_connections": getattr(
                                    pool, "_active_connections", "unknown"
                                ),
                                "max_connections": getattr(
                                    pool, "_max_connections", "unknown"
                                ),
                                "keepalive_connections": getattr(
                                    pool, "_keepalive_connections", "unknown"
                                ),
                            }
                        ...
                        if hasattr(client, "_limits"):
                            ...
                            metrics["client_info"]["limits"] = {
                                "max_keepalive_connections": getattr(
                                    limits, "max_keepalive_connections", "unknown"
                                ),
                                "max_connections": getattr(
                                    limits, "max_connections", "unknown"
                                ),
                                "keepalive_expiry": getattr(
                                    limits, "keepalive_expiry", "unknown"
                                ),
                            }
            ...
            ) + self._connection_pool_stats.get("successful_requests", 0)
            success_rate = 1.0
            if total_requests > 0:
            ...
            metrics["health_indicators"] = {
                ...
                "pool_timeout_rate": (
                    self._connection_pool_stats["pool_timeouts"]
                ...
                "cleanup_frequency": (
                    time.time()
                    - self._connection_pool_stats.get("last_cleanup", time.time())
                )
                / 60,  # 分鐘
    ...
    async def _robust_telegram_request(
        self, request_func, context: str, max_retries: int = None
    ):
        ...
        for attempt in range(max_retries + 1):
            try:
                # 檢查連接池狀態
                if (
                    hasattr(self, "_connection_pool_stats")
                    and self._connection_pool_stats.get("pool_timeouts", 0) > 10
                ):
                    # 如果連接池超時過多，先清理
                ...
                if hasattr(e, "__class__") and "TelegramError" in str(type(e)):
                    ...
                    self.logger.info(
                        f"🔄 {context} 重試 {attempt + 1}/{max_retries}，等待 {wait_time:.1f} 秒..."
                    )
                    await asyncio.sleep(wait_time)
                    ...
                    self.logger.warning(
                        f"⚠️ {context} 意外錯誤，重試 {attempt + 1}/{max_retries}: {e}"
                    )
                    await asyncio.sleep(wait_time)
