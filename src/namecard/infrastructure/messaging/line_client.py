...
class LineBotApiHandler:
    ...
    def _handle_api_error(
        self, error: LineBotApiError, context: str = ""
    ) -> Dict[str, Any]:
        """處理 LINE Bot API 錯誤"""
    ...
    def safe_reply_message(
        self, reply_token: str, message: str, max_retries: int = 2
    ) -> Dict[str, Any]:
        """安全發送回覆訊息，包含錯誤處理"""
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                error_result = self._handle_api_error(
                    e, f"reply_message attempt {attempt + 1}"
                )
    ...
    def safe_push_message(
        self, user_id: str, message: str, max_retries: int = 2
    ) -> Dict[str, Any]:
        """安全發送推播訊息，包含錯誤處理"""
        if not self._check_quota_status():
            return {
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                error_result = self._handle_api_error(
                    e, f"push_message attempt {attempt + 1}"
                )
    ...
    def safe_get_message_content(self, message_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "message": "內容獲取成功", "content": content}
    ...
    def get_status_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "quota_reset_time": (
                self.quota_reset_time.isoformat() if self.quota_reset_time else None
            ),
