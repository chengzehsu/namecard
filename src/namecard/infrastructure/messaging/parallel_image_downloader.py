...
class ParallelImageDownloader:
                ...
                try:
                    ...
                    if any(keyword in error_str for keyword in [
                        ...
                        if attempt < max_retries - 1:
                            ...
                            self.logger.warning(f"下載重試 {attempt + 1}/{max_retries} (等待 {wait_time}s): {error_str[:50]}")
    ...
    async def download_batch_images(
        ...
        try:
            ...
            self.logger.info(f"📦 批次下載完成: {len(telegram_files)} 張圖片，耗時 {batch_time:.2f}s")
    ...
    def _update_avg_download_time(self, new_time: float):
        ...
        if total_downloads == 1:
            ...
            self.stats["avg_download_time"] = (
                (current_avg * (total_downloads - 1) + new_time) / total_downloads
            )
