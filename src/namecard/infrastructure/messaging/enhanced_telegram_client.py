...
from .async_message_queue import AsyncMessageQueue, BatchContext, MessagePriority
...
class EnhancedTelegramBotHandler(TelegramBotHandler):
    ...
    def __init__(
        ...
        self.logger.info(
            f"   - 佇列系統: {'啟用' if self.enable_message_queue else '停用'}"
        )
    ...
    def _should_use_queue(
        self, priority: MessagePriority, text: str, context: Optional[str]
    ) -> bool:
        """智能決策：是否使用佇列系統"""
        ...
        if any(
            ...
            for keyword in ["錯誤", "失敗", "error", "failed", "❌"]
        ):
            return False
    ...
    async def send_progress_update(
        ...
        return await self.send_batch_message(
            chat_id=chat_id, text=text, context=context, message_type="batch_progress"
        )
...
class EnhancedBatchContext:
    ...
    async def send_card_complete(
        self, chat_id: Union[int, str], card_info: Dict[str, Any]
    ) -> str:
        """發送名片完成訊息"""
    ...
    async def send_progress(
        self, chat_id: Union[int, str], current: int, total: int
    ) -> str:
        """發送進度更新"""
        return await self.send_message(
            chat_id=chat_id,
...
class SimpleBatchContext:
    ...
    async def send_message(
        ...
        result = await self.handler.safe_send_message(
            chat_id=chat_id, text=text, parse_mode=parse_mode, context=self.context_name
        )
        return result.get("message_id", "")
...
def create_enhanced_telegram_handler(
    enable_queue: bool = True, queue_workers: int = 8, batch_size: int = 5, **kwargs
    ...
    return EnhancedTelegramBotHandler(
        enable_message_queue=enable_queue, queue_config=queue_config
    )
