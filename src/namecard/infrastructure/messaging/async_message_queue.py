...
class AsyncMessageQueue:
    ...
    async def enqueue_message(
        ...
        if queue.full():
            self.logger.warning(f"⚠️ 佇列已滿 ({message.priority.name})，丟棄訊息: {message.message_id}")
            return message.message_id
        ...
        self.logger.debug(f"📥 訊息已加入佇列: {message.message_id} (優先級: {priority.name})")
        return message.message_id
    ...
    def _merge_card_processing_messages(self, messages: List[QueuedMessage]) -> QueuedMessage:
        """合併名片處理完成訊息"""
        base_message = messages[0]
        count = len(messages)
        ...
        for msg in messages:
            ...
            if "👤" in msg.text and "(" in msg.text:
                ...
                end = msg.text.find("\n", start) if "\n" in msg.text[start:] else len(msg.text)
    ...
    async def _process_message(self, message: QueuedMessage, worker_name: str):
        ...
        try:
            ...
            self.logger.debug(f"✅ 訊息處理成功: {message.message_id} ({processing_time:.2f}s) - {worker_name}")
            ...
            if message.retry_count < message.max_retries:
                ...
                self.logger.info(f"🔄 訊息重試 {message.retry_count}/{message.max_retries} (延遲 {retry_delay}s): {message.message_id}")
    ...
    async def _adjust_concurrency(self, error_rate: float):
        ...
        elif error_rate < 0.1 and self.stats["total_processed"] > self.current_workers * 20:
            # 錯誤率 < 10% 且處理量足夠，增加併發數
        ...
        if new_workers != old_workers:
            ...
            self.logger.info(f"🔧 併發數調整: {old_workers} → {new_workers} (錯誤率: {error_rate:.1%})")
    ...
    def _get_queue_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "error_rate": (
                ...
                if len(self.error_rate_window) >= 20 else 0
            )
        }
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        is_healthy = (
            self.is_running and
            stats["error_rate"] < 0.5 and
            sum(stats["queue_sizes"].values()) < self.max_queue_size * 0.8
        )
