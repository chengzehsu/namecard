...
class HighPerformanceCardProcessor:
    ...
    async def optimize_image_for_ai(
        self, image_bytes: bytes, metadata: ImageMetadata
    ) -> bytes:
        """異步優化圖片以提升 AI 處理速度"""
    ...
    async def _call_gemini_api_async(
        self, image_bytes: bytes, prompt: str, max_retries: int = 3
    ) -> str:
        """異步調用 Gemini API"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 暫時性錯誤重試
                if any(
                    keyword in error_str for keyword in ["500", "502", "503", "timeout"]
                ):
                    if attempt < max_retries - 1:
                        wait_time = (2**attempt) + 0.1
                        self.logger.warning(
                            f"暫時性錯誤，{wait_time}s 後重試: {error_str[:100]}"
                        )
    ...
    async def process_single_card_fast(
        self, image_bytes: bytes, enable_cache: bool = True
    ) -> ProcessingResult:
        """高速處理單張名片"""
    ...
    async def process_batch_multi_card_fast(
        ...
        try:
            ...
            if enable_cache and batch_size <= 5:  # 只對小批次啟用快取
                ...
                cached_result = await self.cache.get_cached_result_async(
                    batch_cache_key
                )
            ...
            # 添加批次處理指令
            batch_parts.append({"role": "user", "content": batch_prompt})
            ...
            ai_time = time.time() - ai_start
            self.logger.info(
                f"⚡ 批次 AI 處理完成: {ai_time:.2f}s ({batch_size} 張圖片)"
            )
            ...
            self.processing_stats["batch_processed"] = (
                self.processing_stats.get("batch_processed", 0) + 1
            )
...
"""
    ...
    async def _send_batch_ai_request(
        self, session: aiohttp.ClientSession, batch_parts: List[Dict]
    ) -> str:
        """發送批次 AI 請求"""
    ...
    def _validate_batch_result(
        self, parsed_data: Dict[str, Any], expected_images: int
    ) -> bool:
        """驗證批次結果"""
...
async def benchmark_processing_speed(
    processor: HighPerformanceCardProcessor, image_bytes: bytes, iterations: int = 5
):
    """基準測試處理速度"""
    times = []

    for i in range(iterations):
        result = await processor.process_single_card_fast(
            image_bytes, enable_cache=(i == 0)
        )
