...
from src.namecard.infrastructure.ai.async_card_processor import (
    AsyncCardProcessor, ProcessingPriority, ProcessingMetadata
)
...
class OptimizedAIService:
    ...
    def __init__(
        ...
        self.notion_manager = AsyncNotionManager(
            max_concurrent=max_concurrent_notion
        )
    ...
    async def process_image(
        ...
        try:
            ...
            # 檢查是否需要保存到 Notion
            should_save = save_to_notion if save_to_notion is not None else self.auto_notion_save
            notion_results = []
            ...
            # 更新統計
            self._update_service_stats(processing_time, True, ai_metadata.cache_hit, len(notion_results) > 0)
    ...
    async def _save_cards_to_notion(
        ...
        try:
            return await self.notion_manager.create_batch_records(
                cards_data=cards,
                image_bytes_list=[image_bytes] * len(cards)
            )
        except Exception as e:
        ...
        try:
            ...
            if cards_to_save:
                await self.notion_manager.create_batch_records(
                    cards_data=cards_to_save,
                    image_bytes_list=image_bytes_to_save
                )
