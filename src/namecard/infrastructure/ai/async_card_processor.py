...
class AsyncCardProcessor:
    ...
    def __init__(self, max_concurrent: int = 10, enable_cache: bool = True):
        ...
        self.api_keys = [
            ...
            Config.GOOGLE_API_KEY_FALLBACK
        ]
...
"""
    ...
    async def _process_with_timeout(
        ...
        return await asyncio.wait_for(
            self._process_image_core(image_bytes, metadata),
            timeout=timeout
        )
    ...
    async def _normalize_address_async(self, card: Dict[str, Any]):
        ...
        def normalize():
            ...
            if result["warnings"]:
                ...
                card["notes"] = f"{current_notes}; {warnings_text}" if current_notes else warnings_text
    ...
    async def _get_available_api_key(self) -> str:
        ...
        for i in range(len(self.api_keys)):
            ...
            if api_key and not self.api_quota_status.get(api_key, {}).get("exhausted", False):
    ...
    def _update_processing_stats(self, metadata: ProcessingMetadata):
        ...
        new_avg = (
            (current_avg * (self.processing_stats["total_processed"] - 1) + 
             metadata.processing_duration) / self.processing_stats["total_processed"]
        )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            available_keys = sum(
                ...
                if key and not self.api_quota_status.get(key, {}).get("exhausted", False)
            )
