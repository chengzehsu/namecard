...
class UltraFastProcessor:
    ...
    async def process_telegram_photos_batch_ultra_fast(
        ...
        try:
            self.logger.info(
                f"🚀 開始真正批次處理 {image_count} 張圖片 (用戶 {user_id})"
            )
            ...
            download_results, user_context = await asyncio.gather(
                asyncio.gather(*download_tasks), user_context_task
            )
            ...
            self.logger.info(
                f"📥 成功下載 {len(successful_downloads)}/{image_count} 張圖片"
            )
            ...
            ai_result = await self.ai_processor.process_batch_multi_card_fast(
                batch_image_data, enable_cache=True, batch_size=len(batch_image_data)
            )
            ...
            if failed_downloads:
                self.logger.warning(
                    f"⚠️ {len(failed_downloads)} 張圖片下載失敗，將在結果中標記"
                )
            ...
            # === 效能評估 ===
            performance_grade = self._evaluate_batch_performance(
                total_time, image_count
            )
            ...
            efficiency_ratio = (
                time_saved / estimated_individual_time
    ...
    async def process_telegram_photo_ultra_fast(
        self, telegram_file: File, user_id: str, processing_type: str = "single_card"
    ) -> UltraFastResult:
        """
        終極高速處理 Telegram 圖片
    ...
    async def _prepare_user_context(self, user_id: str) -> Dict[str, Any]:
        ...
        return {"user_id": user_id, "preferences": {}, "history_count": 0}

    async def _update_processing_stats(
        self, ai_result: ProcessingResult, user_context: Dict[str, Any]
    ):
        """更新處理統計（異步）"""
        # 異步更新各種統計資訊
    ...
    async def _analyze_performance_grade(
        self, total_time: float, optimizations: List[str]
    ) -> str:
        """分析效能等級"""
        if total_time < 5.0:
            return "S"  # 超級快速
    ...
    def _count_parallel_tasks(self, optimizations: List[str]) -> int:
        """計算並行任務數量"""
        parallel_indicators = ["parallel", "concurrent", "async", "batch"]
    ...
    def _update_batch_stats(
        self, performance_grade: str, total_time: float, image_count: int
    ):
        """🚀 Phase 5: 更新批次處理統計"""
        # 更新基礎統計
    ...
    async def process_batch_ultra_fast(
        self, telegram_files: List[File], user_id: str, max_concurrent: int = 3
    ...
    def get_performance_dashboard(self) -> Dict[str, Any]:
        ...
        # 最近效能趨勢
        recent_history = (
            self.processing_history[-20:] if self.processing_history else []
        )
        recent_avg_time = (
        ...
        return {
            ...
            "performance_grades": {
                grade: f"{ratio:.1%}" for grade, ratio in grade_distribution.items()
            },
            "recent_performance": {
    ...
    async def benchmark_ultimate_performance(
        self, test_file: File, iterations: int = 5
    ) -> Dict[str, Any]:
        """終極效能基準測試"""
        ...
        benchmark_results = {
            ...
            "timing": {
                ...
                "cached_run": (
                    f"{results[1]['total_time']:.2f}s" if len(results) > 1 else "N/A"
                ),
            },
            "performance_grades": {grade: grades.count(grade) for grade in set(grades)},
            "detailed_results": results,
        }

        self.logger.info(
            f"🏆 基準測試完成 - 平均時間: {benchmark_results['timing']['avg_time']}"
        )
...
async def ultra_fast_process_telegram_image(
    telegram_file: File, user_id: str, processing_type: str = "single_card"
) -> UltraFastResult:
    """便利函數：終極高速處理"""
