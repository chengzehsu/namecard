...
class PerformanceMonitor:
    ...
    def __init__(self, max_history_size: int = 10000):
        ...
        self.hourly_stats = defaultdict(lambda: {
            'requests': 0, 'avg_time': 0.0, 'error_rate': 0.0
        })
    ...
    def _update_real_time_stats(self, metrics: ProcessingMetrics):
        ...
        hour_stats['avg_time'] = (
            (current_avg * (total_requests - 1) + metrics.processing_time) / total_requests
        )
    ...
    def get_performance_summary(self, time_range_minutes: int = 60) -> Dict[str, Any]:
        ...
        p95_processing_time = (
            ...
            if len(processing_times) > 10 else avg_processing_time
        )
        p99_processing_time = (
            statistics.quantiles(processing_times, n=100)[98]  # 99th percentile
            if len(processing_times) > 100 else avg_processing_time
        )
    ...
    def _assess_health_status(self, recent_metrics: List[ProcessingMetrics]) -> Dict[str, Any]:
        """評估系統健康狀態"""
        if not recent_metrics:
        ...
        if avg_response_time > (self.performance_thresholds['max_response_time_ms'] / 1000):
    ...
    def _get_recent_summary(self, count: int = 10) -> Dict[str, Any]:
        """獲取最近處理的摘要"""
        recent = list(self.processing_history)[-count:] if self.processing_history else []
        
        return {
            'count': len(recent),
            'avg_time_ms': round(statistics.mean(m.processing_time for m in recent) * 1000, 2) if recent else 0,
            'success_rate': sum(1 for m in recent if m.success) / len(recent) if recent else 0,
            'cache_hit_rate': sum(1 for m in recent if m.cache_hit) / len(recent) if recent else 0
        }
    ...
    async def generate_performance_report(
        ...
        for metric in relevant_metrics:
            hour_key = datetime.fromtimestamp(metric.start_time).strftime('%Y-%m-%d %H:00')
        ...
        for hour, data in hourly_breakdown.items():
            hourly_stats[hour] = {
                ...
                'avg_response_time_ms': round((data['total_time'] / data['requests']) * 1000, 2),
        ...
        report = {
            ...
            'executive_summary': {
                ...
                'avg_processing_time_ms': round(statistics.mean(processing_times) * 1000, 2),
                'total_processing_hours': round(sum(processing_times) / 3600, 2),
                'peak_hour': max(hourly_stats.items(), key=lambda x: x[1]['requests']),
                'slowest_hour': max(hourly_stats.items(), key=lambda x: x[1]['avg_response_time_ms'])
            },
            'performance_breakdown': {
                'response_time_percentiles': {
                    'p50': round(statistics.median(processing_times) * 1000, 2),
                    'p90': round(statistics.quantiles(processing_times, n=10)[8] * 1000, 2) if len(processing_times) > 10 else 0,
                    'p95': round(statistics.quantiles(processing_times, n=20)[18] * 1000, 2) if len(processing_times) > 20 else 0,
                    'p99': round(statistics.quantiles(processing_times, n=100)[98] * 1000, 2) if len(processing_times) > 100 else 0
                },
                'quality_metrics': {
                    quality: sum(1 for m in relevant_metrics if m.result_quality == quality)
                ...
                'cache_efficiency': {
                    ...
                    'cache_hit_rate': round(sum(1 for m in relevant_metrics if m.cache_hit) / total_requests, 4)
                }
            },
    ...
    def _generate_recommendations(self, metrics: List[ProcessingMetrics]) -> List[str]:
        ...
        p95_time = statistics.quantiles(processing_times, n=20)[18] if len(processing_times) > 20 else avg_time
