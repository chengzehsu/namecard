...
class SmartCacheManager:
        ...
        if key in self.disk_cache_index:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    if datetime.now() < created_at + timedelta(seconds=file_info['ttl']):
    ...
    async def _save_disk_cache_index(self):
        ...
        try:
            async with aiofiles.open(index_file, 'w', encoding='utf-8') as f:
                await f.write(json.dumps(self.disk_cache_index, indent=2, ensure_ascii=False))
        except Exception as e:
            print(f"⚠️ 儲存磁碟快取索引失敗: {e}")

    def generate_image_cache_key(self, image_bytes: bytes, processing_options: Dict[str, Any] = None) -> str:
        """
        ...
        return {
            ...
            'capacity': {
                'memory': {
                    ...
                    'usage_percentage': round(memory_size / self.max_memory_size * 100, 2),
    ...
    async def cleanup_expired(self):
        ...
        if expired_keys or disk_expired:
            print(f"🧹 清理過期快取: 記憶體 {len(expired_keys)} 項，磁碟 {len(disk_expired)} 項")
