...
class AsyncBatchService:
    ...
    def __init__(
        self, card_processor: AsyncCardProcessor, max_global_concurrent: int = 50
    ):
        """
        初始化異步批次服務
    ...
    async def _cleanup_expired_sessions(self):
        ...
        for session_id, session in self.active_sessions.items():
            ...
            ).total_seconds() > session.timeout_minutes * 60:
    ...
    async def process_batch(
        ...
        # 獲取待處理項目
        pending_items = [
            item for item in session.items if item.status == BatchStatus.PENDING
        ]
        ...
        result_summary = {
            ...
            "items_per_second": (
                len(pending_items) / processing_time if processing_time > 0 else 0
            ),
        }
    ...
    async def _process_batch_item_with_semaphore(
        self, session_id: str, item: BatchItem, semaphore: asyncio.Semaphore
    ):
        """帶信號量的批次項目處理"""
    ...
    async def get_batch_status(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        completed_items = status_counts.get("completed", 0) + status_counts.get(
            "failed", 0
        )
    ...
    async def get_service_stats(self) -> Dict[str, Any]:
        ...
        avg_processing_time = (
            ...
            if self.stats["total_items_processed"] > 0
            else 0.0
        )
        ...
        try:
            ...
            return {
                ...
                "cleanup_task_running": not (
                    self.cleanup_task and self.cleanup_task.done()
                ),
            }
