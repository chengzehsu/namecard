...
class UnifiedResultFormatter:
    ...
    def format_batch_result(self, result: BatchProcessingResult) -> str:
        ...
        if result.successful_results:
            message_parts.extend(self._format_successful_results(result.successful_results))
    ...
    def _format_failed_results(self, results: List[SingleCardResult]) -> List[str]:
        ...
        for i, result in enumerate(results, 1):
            error_summary = self._get_error_summary(result.error_message, result.error_type)
    ...
    def _get_error_summary(self, error_message: Optional[str], error_type: Optional[str]) -> str:
        """將技術錯誤訊息轉換為用戶友好的摘要"""
    ...
    def _generate_suggestions(self, result: BatchProcessingResult) -> str:
        ...
        # 分析失敗原因
        blur_count = sum(1 for r in result.failed_results 
                        if r.error_message and "模糊" in r.error_message.lower())
        dark_count = sum(1 for r in result.failed_results 
                        if r.error_message and "暗" in r.error_message.lower())
        network_count = sum(1 for r in result.failed_results 
                           if r.error_message and ("network" in r.error_message.lower() or 
                                                 "connection" in r.error_message.lower()))
        quota_count = sum(1 for r in result.failed_results 
                         if r.error_message and ("quota" in r.error_message.lower() or 
                                               "limit" in r.error_message.lower()))
    ...
    def format_single_image_result(self, result: SingleCardResult, is_batch: bool = False) -> str:
        """
        格式化單張圖片結果（兼容現有邏輯）
