...
class MultiCardProcessor:
    ...
    def _analyze_and_recommend_action(
    ...
    def _handle_single_card_decision(
        self, card_data: Dict[str, Any], card: Dict[str, Any]
    ) -> Dict[str, Any]:
        """處理單張名片的決策邏輯"""
    ...
    def _handle_multi_card_decision(
        self, card_data: Dict[str, Any], cards: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """處理多張名片的決策邏輯"""
    ...
    def get_cards_for_processing(
        self, card_data: Dict[str, Any], user_choice: str
    ) -> List[Dict[str, Any]]:
        """根據用戶選擇獲取需要處理的名片"""
        try:
            cards = card_data.get("cards", [])
