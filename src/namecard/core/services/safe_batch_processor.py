...
# 導入修復的組件
from src.namecard.infrastructure.messaging.enhanced_telegram_client import EnhancedTelegramBotHandler
...
class SafeBatchProcessor:
    ...
    async def process_batch_safely(
        ...
        self.logger.info(f"🚀 開始安全處理批次 - 用戶: {user_id}, 圖片數: {total_images}")
        
        try:
            # 使用連接池安全上下文（如果可用）
            if (self.config.use_connection_pool_cleanup and 
                self.enhanced_telegram_handler and 
                hasattr(self.enhanced_telegram_handler, '_connection_cleanup_context')):
        ...
        self.logger.info(f"✅ 批次處理完成 - 成功: {len(batch_result.successful_results)}/{total_images}, 用時: {total_processing_time:.1f}秒")
    ...
    async def _process_single_image_safe(
        ...
        async with semaphore:  # 控制並發數量
            ...
            try:
                self.logger.debug(f"🔍 開始處理圖片 {image_index} - 用戶: {image.user_id}")
                ...
                if self.config.enable_ultra_fast and self.ultra_fast_processor:
                    try:
                        result = await self._try_ultra_fast_processing(image, image_index)
                        if result.status == ProcessingStatus.SUCCESS:
                            self.stats["ultra_fast_usage"] += 1
                            return result
                        else:
                            self.logger.debug(f"超高速處理失敗，降級到傳統處理: {result.error_message}")
                    except Exception as e:
    ...
    async def _save_to_notion(self, card_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """保存名片數據到Notion"""
        ...
        try:
            ...
            notion_page = await asyncio.get_event_loop().run_in_executor(
                None,
                self.notion_manager.create_namecard_page,
                card_data
            )
            return notion_page
        except Exception as e:
