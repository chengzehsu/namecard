...
# 註冊事件處理器（在所有函數定義完成後）
if "handler" in globals() and handler:
    try:
        ...
        log_message("✅ 事件處理器註冊成功")
        log_message("📱 LINE Bot 準備接收訊息")
    except Exception as e:
        log_message(f"⚠️ 事件處理器註冊失敗: {e}", "WARNING")
        log_message("💡 應用將以基礎模式啟動（無法處理 LINE 訊息）", "WARNING")
else:
    log_message("⚠️ LINE Bot handler 未初始化，跳過事件註冊", "WARNING")
