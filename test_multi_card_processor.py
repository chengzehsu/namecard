...
class MultiCardTestSuite:
    ...
    def test_name_card_processor(self):
        ...
        try:
            ...
            multi_format = processor._convert_single_card_to_multi_format(single_card_data)
            self.assert_true(
                multi_format.get("card_count") == 1,
                "單一名片應該轉換為 card_count = 1"
            )
            self.assert_true(
                len(multi_format.get("cards", [])) == 1,
                "應該包含一張名片資料"
            )
    ...
    def test_user_interaction_handler(self):
        ...
        try:
            ...
            # 測試無會話狀態
            self.assert_false(
                handler.has_pending_session(user_id),
                "新用戶不應該有待處理會話"
            )
    ...
    def test_multi_card_workflow(self):
        ...
        try:
            ...
            # 多張名片應該需要用戶選擇
            self.assert_true(
                result.get("action_required") == True,
                "多張名片應該需要用戶選擇"
            )
            
            self.assert_true(
                len(result.get("user_options", [])) > 0,
                "應該提供用戶選項"
            )
    ...
    def test_quality_assessment(self):
        ...
        try:
            ...
            confidence = card_processor._calculate_single_card_confidence(high_quality_card)
            self.assert_true(
                confidence >= 0.8,
                f"高品質名片信心度應該 >= 0.8，實際: {confidence}"
            )
            ...
            confidence = card_processor._calculate_single_card_confidence(low_quality_card)
            self.assert_true(
                confidence < 0.8,
                f"低品質名片信心度應該 < 0.8，實際: {confidence}"
            )
    ...
    def test_user_choice_handling(self):
        ...
        try:
            ...
            self.assert_true(
                result.get("action") == "process_all_cards",
                "數字1應該對應第一個選項"
            )
            ...
            self.assert_true(
                result.get("action") == "retake_photo",
                "重新拍攝應該觸發重拍動作"
            )
            ...
            self.assert_true(
                result.get("action") == "invalid_choice",
                "無效選擇應該返回錯誤"
            )
...
def run_integration_test():
    ...
    try:
        ...
        # 1. 創建多名片會話
        choice_message = interaction_handler.create_multi_card_session(user_id, mock_analysis)
        print(f"✅ 會話創建成功，訊息長度: {len(choice_message)}")
        
        # 2. 模擬用戶選擇
        choice_result = interaction_handler.handle_user_choice(user_id, "2")  # 選擇第二個選項
