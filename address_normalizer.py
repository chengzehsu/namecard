...
class AddressNormalizer:
    ...
    def normalize_address(self, address: str) -> Dict[str, Any]:
        ...
        return self._create_result(
            normalized_address, address, normalized_components, confidence, warnings
        )
    ...
    def _normalize_components(self, components: Dict[str, str]) -> Dict[str, str]:
        ...
        if road:
            ...
            road = re.sub(
                r"([0-9])段",
                lambda m: f"{self.number_mapping.get(m.group(1), m.group(1))}段",
                road,
            )
    ...
    def _calculate_confidence(
        self, original_components: Dict[str, str], normalized_components: Dict[str, str]
    ) -> float:
        """計算正規化信心度"""
        score = 0.0
