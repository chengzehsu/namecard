...
class OptimizedNameCardBot:
    ...
    async def process_batch_namecards(
        ...
        self.processing_stats['cache_hits'] += batch_metadata['processing_summary']['cached']
        self.processing_stats['errors'] += batch_metadata['processing_summary']['failed']
        
        # 生成批次摘要
        summary = self._generate_batch_summary(results, batch_metadata)
        
        print(f"✅ 批次處理完成: {user_id} "
    ...
    def _generate_user_response(self, result: Dict[str, Any], metadata: Dict[str, Any]) -> str:
        """生成用戶友善的回應訊息"""
