    ...
    try:
        ...
        async with await create_enhanced_telegram_handler(enable_queue=True) as handler:
            ...
            result = await handler.safe_send_message(
                chat_id="123456789", text="Hello! 這是一條測試訊息"
            )
            print(f"✅ 基本訊息發送結果: {result}")

            # 緊急訊息（高優先級，不排程）
            result = await handler.send_urgent_message(
                chat_id="123456789", text="🚨 這是緊急訊息，需要立即發送！"
            )
            ...
            for i in range(100):  # 模擬100條訊息
                if i % 10 == 0:
                    ...
                    task = handler.send_urgent_message(
                        chat_id="123456789", text=f"🚨 緊急訊息 #{i}"
                    )
                elif i % 5 == 0:
                    # 每5條訊息中有1條完成訊息
                    task = handler.send_completion_message(
                        chat_id="123456789", text=f"✅ 完成訊息 #{i}"
                    )
                else:
                    # 其他為批次訊息
                    task = handler.send_batch_message(
                        chat_id="123456789", text=f"📝 批次訊息 #{i}"
                    )
...
async def example_4_batch_integration():
    ...
    try:
        from src.namecard.infrastructure.messaging.batch_message_integration import (
            create_batch_processing_system,
        )
        ...
        async with await create_enhanced_telegram_handler(enable_queue=True) as handler:
            ...
            for i in range(20):
                await handler.safe_send_message(
                    chat_id="123456789", text=f"測試訊息 #{i}"
                )
