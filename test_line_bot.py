...
class LineBotTester:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if notion_test.get("success"):
                    self.log_test("Notion 連接", True, f"資料庫: {notion_test.get('database_title', 'Unknown')}")
                else:
    ...
    def test_webhook_endpoint(self):
        ...
        try:
            ...
            response = requests.post(f"{self.base_url}/callback", 
                                   json={"test": "data"}, 
                                   timeout=10)
            if response.status_code == 400:
                self.log_test("Webhook POST", True, "正確拒絕無效請求")
            else:
                self.log_test("Webhook POST", False, f"HTTP {response.status_code} (預期 400)")
        except Exception as e:
    ...
    def generate_report(self):
        ...
        if failed_tests == 0:
            ...
            print("   🔧 請修復失敗的測試項目")
            print("   📝 檢查環境變數和依賴包安裝")
            if any("連接" in test["test"] for test in self.test_results if not test["success"]):
