...
class TestOptimizedAIPerformance:
    ...
    async def ai_service(self):
        ...
        service = await create_optimized_ai_service(
            max_concurrent=5, cache_memory_mb=50, cache_disk_mb=100, auto_start=True
        )
    ...
    async def test_concurrent_request_performance(
        self, ai_service, mock_image_data, mock_ai_response
    ):
        """測試併發請求效能"""
        ...
        with patch("google.generativeai.GenerativeModel") as mock_model:
            ...
            # 效能基準測試
            assert (
                len(successful_results) >= concurrent_requests * 0.95
            ), "成功率低於95%"
    ...
    async def test_cache_performance_impact(self, ai_service, mock_image_data):
        ...
        with patch("google.generativeai.GenerativeModel") as mock_model:
            ...
            result1, metadata1 = await ai_service.process_image(
                image_bytes=test_image, enable_cache=True, user_id="cache_test_user"
            )
            ...
            print(f"\n⚡ 快取效能測試結果:")
            print(
                f"   - 第一次請求時間: {first_request_time:.3f}s (快取: {metadata1['cache_hit']})"
            )
            print(
                f"   - 第二次請求時間: {second_request_time:.3f}s (快取: {metadata2['cache_hit']})"
            )
            ...
            assert (
                second_request_time < first_request_time * 0.1
            ), "快取請求應該顯著更快"
    ...
    async def test_batch_processing_efficiency(self, ai_service, mock_image_data):
        ...
        with patch("google.generativeai.GenerativeModel") as mock_model:
            ...
            results, batch_metadata = await ai_service.process_batch(
                image_batch=batch_images, max_concurrent=5, user_id="batch_test_user"
            )
            ...
            for image in batch_images:
                result, _ = await ai_service.process_image(
                    image_bytes=image, user_id="individual_test_user"
                )
            ...
            print(
                f"   - 批次成功率: {batch_metadata['processing_summary']['successful']}/{batch_size}"
            )
            ...
            assert (
                efficiency_gain > 1.5
            ), f"批次處理效率提升不足: {efficiency_gain:.1f}x"
            assert (
                batch_metadata["processing_summary"]["successful"] >= batch_size * 0.9
            ), "批次成功率低於90%"
    ...
    async def test_service_health_monitoring(self, ai_service):
        ...
        print(
            f"   - 可用 API Keys: {service_status['quota_status']['summary']['available_keys']}"
        )

        # 驗證健康狀態
        assert service_status["service_info"]["running"], "服務應該處於運行狀態"
        assert (
            service_status["quota_status"]["summary"]["available_keys"] > 0
        ), "應該有可用的 API Keys"
        assert health_check["status"] in [
            "healthy",
            "degraded",
        ], f"健康狀態異常: {health_check['status']}"
    ...
    async def test_performance_optimization(self, ai_service):
        ...
        print(
            f"   - 執行項目: {optimization_result.get('optimizations_performed', [])}"
        )
