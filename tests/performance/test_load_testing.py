...
class TestPerformanceLoad:
    ...
    def test_session_memory_usage(self, enhanced_session_manager):
        ...
        for i in range(num_sessions):
            ...
            # 添加一些處理數據
            for j in range(10):
                enhanced_session_manager.add_processed_card(
                    user_id,
                    {
                        "name": f"測試名片 {j}",
                        "company": f"測試公司 {j}",
    ...
    def test_high_throughput_card_processing(self, batch_manager):
        ...
        ), f"平均處理時間過長: {avg_processing_time:.4f}s"
    ...
    def test_long_running_session_stability(self, enhanced_session_manager):
        ...
        for batch in range(total_operations // operations_per_batch):
            ...
            for i in range(operations_per_batch):
                if i % 2 == 0:
                    enhanced_session_manager.add_processed_card(
                        user_id, {"name": f"Card {batch*operations_per_batch + i}"}
                    )
                else:
                    enhanced_session_manager.add_failed_card(
                        user_id,
...
def test_comparison_batch_managers():
    ...
    print(f"⚖️ 性能比較:")
    print(
        f"   - 原始管理器: {original_time:.4f}s ({num_operations/original_time:.2f} ops/sec)"
    )
    print(
        f"   - 增強管理器: {enhanced_time:.4f}s ({num_operations/enhanced_time:.2f} ops/sec)"
    )
    print(
        f"   - 性能差異: {((enhanced_time - original_time) / original_time * 100):.1f}%"
    )
