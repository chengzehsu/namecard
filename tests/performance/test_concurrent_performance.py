...
class PerformanceTestSuite:
    ...
    async def setup(self):
        ...
        # 初始化 AI 服務
        self.ai_service = OptimizedAIService(max_concurrent=30, cache_memory_mb=500)
    ...
    async def test_single_user_performance(self) -> Dict[str, Any]:
        ...
        for i in range(test_count):
            try:
                ...
                results.append(
                    {"success": False, "processing_time": 0, "error": str(e)}
                )
    ...
    async def test_concurrent_users(
        self, user_count: int = 10, requests_per_user: int = 5
    ) -> Dict[str, Any]:
        """測試並發用戶效能"""
        print(
        ...
        return {
            ...
            "concurrent_efficiency": (successful / total_requests)
            / (total_time / 30),  # 效率指標
        }

    async def _single_user_request(
    ...
    async def test_cache_effectiveness(self) -> Dict[str, Any]:
        ...
        speedup = (
            first_request_time / second_request_time if second_request_time > 0 else 0
        )
        ...
        return {
            ...
            "cache_efficiency": (first_request_time - second_request_time)
            / first_request_time,
        }

    async def test_stress_load(
        self, max_concurrent: int = 50, duration_seconds: int = 30
    ) -> Dict[str, Any]:
        """壓力測試"""
        print(
        ...
        async def stress_worker(worker_id: int):
            ...
            while time.time() < end_time:
                try:
                    ...
                    results.append(
                        {"success": False, "error": str(e), "worker_id": worker_id}
                    )
...
async def main():
    ...
    try:
        ...
        with open(
            f"performance_test_results_{timestamp}.json", "w", encoding="utf-8"
        ) as f:
            import json
