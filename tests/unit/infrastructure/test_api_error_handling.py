...
def test_quota_exceeded_error():
    ...
    with (
        patch.object(handler.api, "reply_message", side_effect=mock_error),
        patch("time.sleep"),
    ):  # 跳過延遲
...
def test_client_error():
    ...
    mock_error = LineBotApiError(
        status_code=400, headers={}, request_id="test-request-id", error=error_obj
    )
...
def test_server_error():
    ...
    mock_error = LineBotApiError(
        status_code=500, headers={}, request_id="test-request-id", error=error_obj
    )
