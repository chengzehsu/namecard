...
class TestTextMessageHandlers:
    ...
    def setup_method(self):
        ...
        with (
            patch("app.line_bot_api", self.mock_line_bot_api),
    ...
    def test_batch_status_command_in_batch(self):
        ...
        self.mock_batch_manager.get_batch_progress_message.return_value = (
            "目前處理了 3 張名片"
        )
        ...
        self.mock_batch_manager.get_batch_progress_message.assert_called_once_with(
            "test_user_id"
        )
        self.mock_line_bot_api.reply_message.assert_called_once()
    ...
    def test_create_pr_command_with_description(self):
        ...
        with patch("app.pr_creator") as mock_pr_creator:
            ...
            # 檢查 PR 創建被調用
            mock_pr_creator.create_instant_pr.assert_called_once_with(
                "添加用戶登入功能"
            )
