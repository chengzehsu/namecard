...
class TestNotionManager:
    ...
    def test_build_properties_complete_data(self, sample_card_data):
        ...
        assert (
            properties["公司名稱"]["rich_text"][0]["text"]["content"] == "測試科技公司"
        )
        ...
        assert (
            properties["取得聯繫來源"]["rich_text"][0]["text"]["content"] == "名片交換"
        )
        assert (
            properties["聯繫注意事項"]["rich_text"][0]["text"]["content"]
            == "專精於軟體開發"
        )
    ...
    def test_build_properties_invalid_email(self):
        """測試無效 email 的處理"""
        data_with_invalid_email = {"name": "測試用戶", "email": "invalid-email-format"}
        ...
        assert (
            "Email 格式錯誤"
            in properties["聯繫注意事項"]["rich_text"][0]["text"]["content"]
        )

    @pytest.mark.unit
    def test_build_properties_invalid_phone(self):
        """測試無效電話號碼的處理"""
        data_with_invalid_phone = {"name": "測試用戶", "phone": "invalid-phone"}
    ...
    def test_create_name_card_record_success(self, sample_card_data):
        ...
        mock_response = {"id": "test_page_id", "url": "https://notion.so/test_page_id"}
        self.notion_manager.client.pages.create.return_value = mock_response

        image_bytes = b"fake_image_data"
        result = self.notion_manager.create_name_card_record(
            sample_card_data, image_bytes
        )
    ...
    def test_create_name_card_record_notion_api_error(self, sample_card_data):
        """測試 Notion API 錯誤處理"""
        # Mock API 錯誤
        self.notion_manager.client.pages.create.side_effect = Exception(
            "Notion API error"
        )
    ...
    @pytest.mark.unit
    @patch("notion_manager.NotionManager._add_image_info_to_page")
    def test_create_name_card_record_image_error(
        self, mock_add_image, sample_card_data
    ):
        """測試圖片處理錯誤的情況"""
        # Mock successful page creation
        mock_response = {"id": "test_page_id", "url": "https://notion.so/test_page_id"}
    ...
    def test_add_image_info_to_page_api_error(self):
        ...
        # Mock API 錯誤
        self.notion_manager.client.blocks.children.append.side_effect = Exception(
            "Block append failed"
        )
    ...
    def test_test_connection_success(self):
        ...
        self.notion_manager.client.databases.query.assert_called_once_with(
            database_id="test_db_id", page_size=1
        )
    ...
    def test_test_connection_failure(self):
        """測試連接測試失敗"""
        # Mock API 錯誤
        self.notion_manager.client.databases.query.side_effect = Exception(
            "Database not found"
        )
