...
class TestRealAPIIntegration:
    ...
    def skip_if_no_api_keys(self):
        """如果沒有 API 金鑰則跳過測試"""
        required_keys = ["GOOGLE_API_KEY", "NOTION_API_KEY", "NOTION_DATABASE_ID"]
    ...
    def test_google_gemini_api_connectivity(self, name_card_processor):
        ...
        try:
            ...
            result = name_card_processor.process_business_card(
                test_image_data, user_id="api_test_user"
            )
    ...
    def test_notion_api_connectivity(self, notion_manager):
        ...
        try:
            ...
            ), f"Notion 記錄創建失敗: {result.get('error')}"
            ...
            assert (
                response_time < 15.0
            ), f"Notion API 響應時間過長: {response_time:.2f}s"
    ...
    def test_api_rate_limiting_behavior(self, name_card_processor):
        ...
        for i in range(5):
            try:
                start_time = time.time()
                result = name_card_processor.process_business_card(
                    test_image_data, user_id=f"rate_limit_test_user_{i}"
                )
                end_time = time.time()
        ...
        rate_limit_errors = [
            e for e in errors if "rate" in e.lower() or "quota" in e.lower()
        ]
    ...
    def test_api_error_recovery(self, name_card_processor):
        ...
        try:
            result = name_card_processor.process_business_card(
                invalid_image_data, user_id="error_recovery_test_user"
            )
    ...
    def test_end_to_end_api_workflow(self, name_card_processor, notion_manager):
        ...
        try:
            ...
            process_result = name_card_processor.process_business_card(
                test_image_data, user_id="e2e_test_user"
            )
            ...
            # 檢查 Notion 結果
            assert (
                notion_result.get("success") == True
            ), f"Notion 存儲失敗: {notion_result.get('error')}"
...
class TestAPIHealthChecks:
    ...
    def test_external_service_availability(self):
        ...
        # 至少一個服務應該可用
        available_services = [
            name for name, info in results.items() if info["available"]
        ]
