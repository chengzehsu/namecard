...
class TestEndToEndWorkflow:
    ...
    def setup_mocks(self):
        ...
        mock_content = Mock()
        mock_content.iter_content.return_value = [
            b"fake_image_chunk1",
            b"fake_image_chunk2",
        ]
    ...
    def test_single_card_processing_complete_workflow(self):
        ...
        # 驗證完整流程
        # 1. 檢查是否下載了圖片
        self.mock_line_api_instance.get_message_content.assert_called_once_with(
            "test_image_message_id"
        )
    ...
    def test_batch_processing_complete_workflow(self):
        ...
        mock_gemini_response2.text = json.dumps(
            {"name": "李四", "company": "另一家公司", "title": "經理"}
        )
    ...
    def test_error_handling_workflow(self):
        """測試錯誤處理工作流程"""
        # 模擬 Gemini API 錯誤
        self.mock_gemini_instance.generate_content.side_effect = Exception(
            "Gemini API 連接失敗"
        )
    ...
    def test_address_normalization_integration(self):
        ...
        mock_gemini_response = Mock()
        mock_gemini_response.text = json.dumps(
            {"name": "王五", "address": "台北信義區信義路5段7號1F"}  # 需要正規化的地址
        )
        self.mock_gemini_instance.generate_content.return_value = mock_gemini_response
