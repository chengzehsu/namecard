...
def test_media_group_logic():
    ...
    try:
        ...
        problematic_patterns = [
            "batch_image_collector.add_image",
            "添加到批次收集器"
        ]
...
def test_batch_collection_logic():
    ...
    try:
        ...
        # 確認媒體群組檢測邏輯
        if "if update.message.media_group_id:" in source and "handle_media_group_message" in source:
            print("✅ 媒體群組檢測邏輯正確")
        else:
            print("❌ 媒體群組檢測邏輯有問題")
            return False
        ...
        # 確認批次收集器只在非媒體群組情況使用
        batch_collection_section = source[source.find("智能批次收集邏輯"):source.find("原有邏輯")]
...
def test_syntax_check():
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "-m", "py_compile", "src/namecard/api/telegram_bot/main.py"],
            capture_output=True,
            text=True
        )
