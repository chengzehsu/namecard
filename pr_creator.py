...
class PRCreator:
    ...
    def create_instant_pr(self, description: str, base_branch: str = "main") -> Dict:
        ...
        try:
            ...
            if not requirements:
                return {"success": False, "error": "Could not parse requirements from description"}
            ...
            pr_result = self._create_pull_request(description, requirements, branch_name, base_branch)
    ...
    def _parse_pr_description(self, description: str) -> List[PRRequirement]:
        ...
        for sentence in sentences:
            ...
            # Determine priority
            priority = 'medium'
            if any(word in sentence for word in ['urgent', 'critical', 'important', 'must']):
                priority = 'high'
            elif any(word in sentence for word in ['minor', 'small', 'simple', 'optional']):
                priority = 'low'
            
            requirements.append(PRRequirement(
                action=action,
                target=target,
    ...
    def _analyze_codebase(self) -> Dict:
        ...
        try:
            ...
            # Analyze main files
            main_files = [f for f in info["files"] if f in ['app.py', 'main.py', 'index.js', 'package.json', 'requirements.txt']]
    ...
    def _generate_code_changes(self, requirements: List[PRRequirement], codebase_info: Dict) -> List[CodeChange]:
        ...
        for req in requirements:
            if req.action == 'add' and 'pr' in req.target.lower():
                ...
                changes.extend(self._generate_pr_functionality_changes(req, codebase_info))
            elif req.action == 'add' and any(word in req.description for word in ['command', 'endpoint', 'api']):
                ...
                changes.extend(self._generate_linebot_enhancement_changes(req, codebase_info))
            else:
                # Generic changes
    ...
    def _generate_pr_functionality_changes(self, req: PRRequirement, codebase_info: Dict) -> List[CodeChange]:
        ...
        if 'app.py' in [os.path.basename(f) for f in codebase_info.get("files", [])]:
            app_file = next((f for f in codebase_info["files"] if f.endswith('app.py')), None)
            if app_file:
    ...
    def _generate_api_endpoint_changes(self, req: PRRequirement, codebase_info: Dict) -> List[CodeChange]:
    ...
    def _generate_linebot_enhancement_changes(self, req: PRRequirement, codebase_info: Dict) -> List[CodeChange]:
    ...
    def _generate_generic_changes(self, req: PRRequirement, codebase_info: Dict) -> List[CodeChange]:
...
'''
    ...
    def _create_feature_branch(self, description: str) -> Optional[str]:
        ...
        try:
            ...
            branch_name = f"feature/{'_'.join(safe_words)}_{datetime.now().strftime('%m%d')}"
...
'''
        ...
        if "elif user_message.lower().startswith('create pr:')" not in content:
            ...
            for i, line in enumerate(lines):
                if 'else:' in line and 'batch_manager.is_in_batch_mode' in ''.join(lines[i+1:i+5]):
                    insert_idx = i
                    break
    ...
    def _commit_changes(self, description: str, requirements: List[PRRequirement]) -> bool:
    ...
    def _create_pull_request(self, description: str, requirements: List[PRRequirement], 
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("Example: python pr_creator.py \"Add user authentication with email verification\"")
        sys.exit(1)
